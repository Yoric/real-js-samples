/* !!! 이 파일을 각 js 파일을 merge하여 만들어집니다. 이파일을 직접 수정하지 마세요 !!! */
/* [common.js] */
// photo scroll ------------------------
...
function openLogin(type, formName, targetUrl, width, height, scroll, refresh, noNonMem) {
	//targetUrl = '/community/AuthShoppingInfoAction.tmall?method=getContentsReporting&contNo=11303&contDclObjClfC=D01&memNM=한글'
	//alert(encodeURIComponent(targetUrl));
	//alert(escape(targetUrl));
	var url = "";
	try {
	...
	/*
	if(type == '4') str = 'width=400,height=550,resizable=no,scrollbars=no,left=100,top=20';
	else str = 'width=400,height=350,resizable=no,scrollbars=no,left=100,top=20';
...
function openLoginLayer(type, formName, targetUrl, width, height, scroll) {
	...
	if(!loginDialog){
		loginDialog = new Ext.BasicDialog("loginDialog", {
				width:405,
				height:380,
				shadow:true,
				minWidth:300,
	...
	btnManager = 'loginBtnManage';
	loginDialog.show(btnManager);
	loginDialog.on('show', function(){
							var formObj = document.popupLoginForm;
							formObj.method = 'post';
							formObj.target = 'loginPopupWindow';
							formObj.type.value = type;
							formObj.formName.value = formName;
							formObj.targetUrl.value = escape(targetUrl);
							formObj.width.value = width;
							formObj.height.value = height;
							formObj.action = '/login/PopupLogin.tmall';
							//formObj.action = '/login/PopupLogin.tmall?type='+type+'&formName='+formName+'&targetUrl='+escape(targetUrl)+'&width='+width+'&height='+height;
							formObj.submit();
					 }
	);
}
...
function writeForm() {
    ...
    var formElement=document.createElement("<form name='popupLoginForm' method='post'/>");
    formElement.insertBefore(createHidden("type",""));
    formElement.insertBefore(createHidden("formName",""));
    formElement.insertBefore(createHidden("targetUrl",""));
    formElement.insertBefore(createHidden("width",""));
    formElement.insertBefore(createHidden("height",""));

    document.body.insertBefore(formElement);

    var divElement=document.createElement("<div id='loginDialog' style='VISIBILITY: hidden; POSITION: absolute; TOP: 0px' />");
	var divElement2=document.createElement("<div class='x-dlg-bd'>");
	var iframeElement=document.createElement("<iframe name='loginPopupWindow' src='about:blank' width='100%' height='100%' frameborder='0' scrolling='auto'>");
    divElement2.insertBefore(iframeElement);
    divElement.insertBefore(divElement2);

    document.body.insertBefore(divElement);

    var spanElement=document.createElement("<span id=loginBtnManage style='margin-top:-2px'/>");
    document.body.insertBefore(spanElement);

	/*
	document.write('<form name=popupLoginForm method=post>');
	document.write('<input type="hidden" name="type" value="">');
	document.write('<input type="hidden" name="formName" value="">');
	document.write('<input type="hidden" name="targetUrl" value="">');
	document.write('<input type="hidden" name="width" value="">');
	document.write('<input type="hidden" name="height" value="">');
	document.write('</form>');

	document.write('<div id=loginDialog style="VISIBILITY: hidden; POSITION: absolute; TOP: 0px">');
	document.write('	<div class=x-dlg-hd>Popup Login</div>');
	document.write('	<div class=x-dlg-bd>');
	document.write('		<div>');
	document.write('			<iframe name=loginPopupWindow src="about:blank" width="100%" height="100%" frameborder="0" scrolling="auto"> </iframe>');
	document.write('		</div>');
	document.write('	</div>');
	document.write('</div>');
	*/
...
function createHidden(n,v) {
  ...
  //o.name="popType";
  //o.value=v;
  var o=document.createElement("<input type='hidden' name='"+n+"'>");
  return o;
}
