...
}function a(f,n){var m=[];
for(var l in f){if(f.hasOwnProperty(l)){m[l]=intel.validate.func(f[l])
}}return m
...
var g=i(window.flashembed,{conf:b,getVersion:function(){var m,f;
...
if(f=="object"&&l.push){f="array"
}switch(f){case"string":l=l.replace(new RegExp('(["\\\\])',"g"),"\\$1");
l=l.replace(/^\s?(\d+\.?\d+)%/,"$1pct");
return'"'+l+'"';
case"array":return"["+a(l,function(o){return g.asString(o)
}).join(",")+"]";
case"function":return'"function()"';
case"object":var m=[];
for(var n in l){if(l.hasOwnProperty(n)){m.push('"'+n+'":'+g.asString(l[n]))
}}return"{"+m.join(",")+"}"
}return String(l).replace(/\s/g," ").replace(/\'/g,'"')
},getHTML:function(o,l){o=i({},o);
var n='<object width="'+o.width+'" height="'+o.height+'" id="'+o.id+'" name="'+o.id+'"';
if(o.cachebusting){o.src+=((o.src.indexOf("?")!=-1?"&":"?")+Math.random())
}if(o.w3c||!h){n+=' data="'+o.src+'" type="application/x-shockwave-flash"'
}else{n+=' classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"'
}n+=">";
if(o.w3c||h){n+='<param name="movie" value="'+o.src+'" />'
...
if(l){for(var f in l){if(l[f]){var q=l[f];
...
n+="</object>";
return n
},isSupported:function(f){return k[0]>f[0]||k[0]==f[0]&&k[1]>=f[1]
}});
var k=g.getVersion();
function d(f,o,n){if(g.isSupported(o.version)){f.innerHTML=g.getHTML(o,n)
}else{if(o.expressInstall&&g.isSupported([6,65])){f.innerHTML=g.getHTML(i(o,{src:o.expressInstall}),{MMredirectURL:location.href,MMplayerType:"PlugIn",MMdoctitle:document.title})
}else{if(!f.innerHTML.replace(/\s/g,"")){var m=((document.location.protocol=="https:")?"https://":"http://");
f.innerHTML="<a href='http://www.adobe.com/go/getflashplayer'><img src='"+m+"www.adobe.com/images/shared/download_buttons/get_flash_player.gif' alt='Get Adobe Flash player' /></a>";
if(f.tagName=="A"){f.onclick=function(){location.href=j
}
}}if(o.onFail){var l=o.onFail.call(this);
if(typeof l=="string"){f.innerHTML=l
}}}}if(h){window[o.id]=document.getElementById(o.id)
}i(this,{getRoot:function(){return f
...
jQuery.fn.flashembed=function(l,f){return this.each(function(){$(this).data("flashembed",flashembed(this,l,f))
})
}
}})();
"use strict";
var intel=intel||{};
intel.pdf=(function(b){var d;
function c(){d=b("#flexpaper-container");
if(d.length){a(d.attr("data-pdf-url"))
}}function a(e){console.log(e);
if(typeof e==="string"){b("#flexpaper-container").FlexPaperViewer({config:{SwfFile:"./scripts/flexpaper/FlexPaperViewer.swf",PDFFile:e}})
}}return{initialize:c}
})(jQuery);
jQuery(document).ready(intel.pdf.initialize);