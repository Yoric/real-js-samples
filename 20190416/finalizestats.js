...
mistats.newsletter =
{
   ...
   getId: function ()
   {
      ...
      cmp = (location.search || '').match(/\&?cid=DM\d+&bid=\d*/);
      if (cmp)
      {
         mistats.setOnce('eVar80', cmp[0].match(/DM\d+/)[0]);
         mistats.setOnce('eVar81', cmp[0].match(/&bid=\d*/)[0].substr(5));
         history.replaceState && (history.replaceState('', document.title, location.pathname + (location.search || '').replace(cmp[0], '').replace(/^[\?&]+/, '?') + (location.hash || '')));
      }
...
mistats.findWidgets = function ()
{
            ...
            if (xt)
            {
               ...
               function bindTracker()
               {
                  ...
                  mistats.bind(cta, 'mousedown', function (pEvent)
                  {
                     ...
                     to = pEvent.target;
                     while (to && !(to.className || '').match(/widget-button|subscribe-button/))
                        to = to.parentNode || null;
   ...
   objs = document.getElementById('newsletter-signUpWidget');
   if (objs)
   {
      mistats.listProp.add(wgts, 'Newsletter CTA');
      mistats.bind(document.getElementById('newsletterSignupSubmit'), 'mousedown', function (pEvent)
      {
         var id;
         var to;
         ...
         id = document.getElementById('s_email_address');
...
mistats.device =
{
   ...
   isWebView: function ()
   {
      return !mistats.device.isFacebook() && mistats.device.userAgent.search(/(ipad|iphone|ipod).+applewebkit.+mobile\/\S+$/) > -1;
   },
...
(function ()
{
   ...
   cid = (location.hash || '').match(/[#&]cid=[^\?#&]+/i) || (location.search || '').match(/[\?&]cid=[^\?#&]+/i);
   if (cid)
   {
...
mistats.sendBeacon = function ()
{
   ...
   function track(pEvent)
   {
      ...
      {
         mistats.setOnce('prop12', 'pageview:' + (pEvent.type === 'timeout' ? 'timeout' : 'aborted'));
         mistats.aborted = true;
//         mistats.aborted = pEvent.type === 'blur' ? false : true;
      ...
      mistats.propScore && (s.eVar30 = 'score:' + mistats.propScore);
   ...
   if (((navigator.userAgent || '').match(/MSIE|msie|rv:11\.0/) && query && path) || s.c_r('mi_ib') == '1')
...
(function ()
{
   ...
   if (!window.fbq)
   {
      ...
      window.fbq('track', 'ActiveSubscriberPageview');
   } else
      window.fbq('track', 'ActiveSubscriberPageview');
...
mistats.tyntid && mistats.isEscenic && mistats.mediaHost && mistats.jsLoader && (mistats.jsLoader(mistats.mediaHost + '/mistats/mi_content_tracker.js'));
...
(function ()
{
   window.addEventListener('mi_launchload', function ()
   {
      ...
      !(window.adobe && window.adobe.target) && (mistats.windowEvent('mistats_adobe'));
   }, false);
})();
