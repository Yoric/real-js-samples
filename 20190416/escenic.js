...
mistats.Hashtags = function ()
{
   function addHashtags(pParent, pHashtag)
   {
      ...
      for (i = 0; i < links.length; i++)
         links[i].href && (links[i].href = links[i].href.replace(/#.*/, '') + '#' + pHashtag);
   };
   ...
   function init()
   {
      ...
      addHashtags(document.getElementById('series-links'), 'storylink=series');
...
mistats.Interactions = function ()
{
   ...
   types =
   {
      'email_cta': {name: 'Newsletter Signup', type: 'newsletter', once: true},
   ...
   function sendCountsNow()
   {
      if (!(count || s.list1))
         return;

      count && (mistats.events.add('event16=' + count));
      s.tl(true, 'o', 'interactions');
   };
...
*/
   ...
   function init(pCount)
   {
      ...
      if (typeof videojs !== 'undefined' && videojs.getPlayers)
      {
         ...
         for (i = 0; i < objs.length; i++)
         {
            player = objs[i].id ? videojs.getPlayers()[objs[i].id] : null;
...
mistats.wordCount = function ()
{
   ...
   for (i = 0; !story && i < objs.length; i++)
      (objs[i].id || '').match(/^content-body/) && (story = objs[i]);
...
mistats.CTAListener = function ()
{
   ...
   {
      waiting = false;
      if (!(document.getElementById('story-cta-widget') && document.getElementById('newsletter-signUpWidget')))
         return;
...
mistats.trackDirectConversion = function ()
{
   ...
   xr.onreadystatechange = function ()
   {
      ...
      page = page
       ...
       +  obj[0]
       + '<script type="text\/javascript" src="//media2.newsobserver.com/mistats/products/escenic_s_code.js"></script>'
       + '<script type="text\/javascript" src="//media2.newsobserver.com/mistats/products/escenic.js"></script>'
       + '<script type="text\/javascript">mistats.isDirectConv = true; window != top && (mistats.events.add(\'event45\'));</script>'
       + '<script type="text\/javascript" src="//media2.newsobserver.com/mistats/finalizestats.js"></script>'
       + '</body></html>';
