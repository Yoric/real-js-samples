...
$j_mediametrics = $.noConflict(true);
/* Initialisation
 ----------------------------------------------- */
if (typeof MediaMetricsInjectClass == "undefined"){
    
var identificator = Math.floor(Math.random() * (9999999999999999 - 10 + 1)) + 10;

$j_mediametrics('#MediaMetricsInject').attr('id', 'MediaMetricsInject-'+identificator);

var MediaMetricsInjectClass = {
    ...
    if (elements.length > 0) {
        for (item1=0; item1<elements.length; item1++) {
         ...
         if (
                ...
                && (!elements[item1].getAttribute('is-used'))
                && (elements[item1].src.indexOf('injectv2.js') > -1)
                && (
         	...
         	elements[item1].setAttribute('is-used', 'true');


         ////////console.log(elements[item1]);
...
break;
    ...
    if (this.variousParams[0].img){
    	var url = '//mediametrics.ru/partner/inject/'+ self.variousParams[0].period +'.' + this.variousParams[0].country.replace('/', '.') + '.js';
		$j_mediametrics.ajax({
		  url: url,
		  success: function(data){
		  	eval(data);
		  	self.widgetLoad(MediaMetricsRating);
		  },
		  error: function() {
		  	self.init();
		  }
...
MediaMetricsInjectClass.widgetLoad = function(dat) {
    var that = this;
...
MediaMetricsInjectClass.widgetInsert = function() {
    ...
    if (this.data.length > 0) {
    	...
    	//////console.log(this.variousParams);


    	for (var t = 0; t < this.variousParams.length; t++) {


      //   if ( this.$el[i].attributes['data-isproceed']==undefined && !w['mediametricsWidgetAttribute'+identificator]){
      //   	w['mediametricsWidgetAttribute'+identificator] = true;

    		// ////////console.log(this.variousParams[i]);
      ...
      ////////console.log(t);
      if (!w['mediametricsWidgetAttribute'+'-'+t+'-'+identificator] || w['mediametricsWidgetAttribute'+'-'+t+'-'+identificator].indexOf('-'+i+'-')==-1)
      {
      	w['mediametricsWidgetAttribute'+'-'+t+'-'+identificator] = true;
      	
    //  	//////console.log(t);

      	//////console.log('виджет ' + 'mediametricsWidgetAttribute'+'-'+t+'-'+identificator + ' построен!');

      	//////console.log(this.variousParams[t]);

      	this.$wrapper = document.createElement('div');
    	this.$wrapper.innerHTML = this.widgetTemplate(t);
    	this.counterEvent();
    	if (this.$el[0].parentNode != null) {
	    	this.$el[0].parentNode.insertBefore(this.$wrapper, this.$el[0].nextSibling);	
    	}
    	// this.$el[0].setAttribute('data-isproceed', 'true');
    	// this.counterEvent();
     }
...
MediaMetricsInjectClass.widgetStyle = function() {
    ...
    this.$block = document.getElementById('mm-block-'+identificator);
     ...
     //   if (this.variousParams[i]==undefined){
      ...
      if (typeof(this.$block)=='object'){
        ...
        if (current_font != this.variousParams[0].fontfamilycurrent) {

            current_font = this.variousParams[0].fontfamilycurrent;



            var head  = document.getElementsByTagName('head')[0];
            var link  = document.createElement('link');
            link.id   = 'main-font-family-' + this.variousParams[0].fontfamily;
            link.rel  = 'stylesheet';
            link.type = 'text/css';
            link.href = 'https://fonts.googleapis.com/css?family=' + this.variousParams[0].fontfamily;
          ...
          this.$block.style.fontSize = this.variousParams[0].fontsize + 'px';
          this.$block.style.fontWeight = this.variousParams[0].fontweight;

          this.$block.style.lineHeight = this.variousParams[0].lineheight + 'px';

          this.$block.style.textAlign = 'left';



          ////console.log(this.variousParams[0].fontalign);

          this.$block.style.padding = '5px';
          this.$block.style.background = this.variousParams[0].transparent == 'true' ? 'transparent' : '#' + this.variousParams[0].bgcolor;
          // Set widget size depends from border width (if defined).

          this.$block.style.borderColor = '#' + this.variousParams[0].bordercolor;
        if (this.variousParams[0].border == 'true' && this.variousParams[0].borderwidth > 0) {
          // Border
          this.$block.style.borderWidth = this.variousParams[0].borderwidth + 'px';


          this.$block.style.padding = '10px';
          // Block
          if (this.variousParams[0].adaptive==false){
            this.$block.style.width = this.variousParams[0].width - (this.variousParams[0].borderwidth*2 + 20) + 'px';
            this.$block.style.height = this.variousParams[0].height - (this.variousParams[0].borderwidth*2 + 20) + 'px';
          }
        } else {
            if (this.variousParams[0].adaptive==false){
                this.$block.style.width = this.variousParams[0].width + 'px';
                this.$block.style.height = this.variousParams[0].height + 'px';
            }
        }
      }
};
...
MediaMetricsInjectClass.widgetTemplate = function(i) {
    ...
    if (this.variousParams[i].adaptive=='true' || this.variousParams[i].adaptive==true){
        style = 'width: 100%; height: 100%;';


        //////////console.log(this.variousParams[i].lineClass, '!!@!@!@!@');

        if (this.variousParams[i].lineClass == "horizontal") { 

        	style += 'min-width: 290px; min-height: 200px;';

        } 
        else {


        	style += 'min-width: 240px; min-height: 200px;';
    ...
    style += 'font-family: '+this.variousParams[i].fontfamilycurrent+', sans-serif;"';

    //////////////console.log(style);

// ////console.log(this.variousParams[blockIndex]);
//     return;
	

    
    return "<div id='mm-block-"+identificator+"' class='mm-body mm-body__fontsize--"+this.variousParams[i].fontsize+" mm-body__fontface--"+this.variousParams[i].fontfamilycurrent+" mm-body__border--"+this.variousParams[i].border+" "+classes+" "+(this.variousParams[i].img=='true'?'mm-body_imaged':'')+"' id='mm-body' style='"+style+" overflow-y: auto;'><div class='mm-body__inner'><div style='text-align: left !important;'>"+ this.widgetRows(i) +"</div></div></div>";
    ...
    for (var index=0;index<data[blockIndex].length;index++) {
        ...
        if (typeof(item) == 'object') {
            ...
            if (this.variousParams[blockIndex].horizontalonewidth != undefined || this.variousParams[blockIndex].horizontaloneheight != undefined){
             ...
             if (this.variousParams[blockIndex].alignment == 'horizontal' && this.variousParams[blockIndex].textposition == 'side') {	
                  row += "width: calc(" + (100/this.variousParams[blockIndex].rows) + "% - 10px) !important; ";
             }
            ...
            if (this.variousParams[blockIndex].img=='true'){
                ...
                spanwidth = 'calc(100% - ' + (parseInt(this.variousParams[blockIndex].imgsize) + 10) + 'px);';

                var clamp = 3;
                if (this.variousParams[blockIndex].lineClass == "fixed") {
                   clamp = 1;
                }


                clamp = parseInt(this.variousParams[blockIndex].visiblerows);

                var float = this.variousParams[blockIndex].place;

                lineheight = parseInt(this.variousParams[blockIndex].lineheight);

                var aWidth = this.variousParams[blockIndex].imgsize + 'px';
             ...
             if (this.variousParams[blockIndex].alignment == 'horizontal' && this.variousParams[blockIndex].textposition == 'side') {
             	...
             	spanWidthToI = spanwidth;

             	//console.log(spanwidth, spanWidthToI);
             }        
             else {
             	if (this.variousParams[blockIndex].alignment == 'horizontal') {
             		spanwidth = '100%';
             ...
             var imgsizeY = (imgsize * 70.91) / 100;


             	if (this.variousParams[blockIndex].forse_height == 'true') {

             		this.variousParams[blockIndex].type = 'img-rect-n';
                ...
                //row += "><a style='color: #"+this.variousParams[blockIndex].linkscolor+";' target='_blank' href='//mediametrics.ru/click;"+ this.variousParams[blockIndex].site +"?http://mediametrics.ru/rating/"+ this.variousParams[blockIndex].country +"/"+ this.variousParams[blockIndex].period +".html?article="+ item[0] +"'><img style='width: "+imgsize+"px; height: "+imgsize+"px; position: relative; float: "+float+";' src='//mediametrics.ru/partner/inject/img/"+ item[0] +".jpg' valign='middle' style='display:inline;' /><span style='float: "+float+"; width: "+spanwidth+"px; height: "+spanheight+"; overflow: hidden; line-height: "+lineheight+"px'>"+item[1]+"</span></a></div>";
                if (item[0]=='000'){
                    row += "><a style='color: #"+this.variousParams[blockIndex].linkscolor+'; ' +(this.variousParams[blockIndex].type=='text-over-image'?'position: relative; width: auto; height: auto;':'')+"; "+(this.variousParams[blockIndex].alignment=='horizontal' && this.variousParams[blockIndex].type!='text-over-image' ? 'width: ' + aWidth + ';' : '')+"' target='_blank' href='"+item[3]+"'><img style='"+(this.variousParams[blockIndex].type=='img-rect-n' ? 'height: ' + imgsizeY + 'px !important;' : '')+"  "+(this.variousParams[blockIndex].type!='text-over-image' ?" width: "+imgsize+"px; color: "+((this.variousParams[blockIndex].type=='img-rect')? "red;" : "blue;") + "height: "+((this.variousParams[blockIndex].type=='img-rect')? "auto;" : imgsize+"px; ") : 'width: ' + widthToI+'px;' + 'height: '+heightToI+'px;')+" position: relative; float: "+float+";' src='" + (this.variousParams[blockIndex].type=='text-over-image'? '//mediametrics.ru/partner/inject/img/w_adv.jpg ' : "//mediametrics.ru/partner/inject/img/" + ((this.variousParams[blockIndex].type=='img-big')? 'b_': (this.variousParams[blockIndex].type=='img-rect' ? 'w_' : (this.variousParams[blockIndex].type=='img-rect-n' ? 'w2_' : ''))) + "adv.jpg" ) + "' valign='middle' style='display:inline;' />"+(this.variousParams[blockIndex].type=='text-over-image'? "<span class='block-text-overflow-image-full' style='width: "+spanwidth+"; max-height: "+(lineheight * clamp)+"px;  text-align: "+this.variousParams[blockIndex].fontalign+" !important; -webkit-line-clamp: "+ clamp + "; position: absolute; left: 0; bottom: 0; text-shadow: #000 2px 2px 5px; color: #fff;  width: "+spanWidthToI+"; font-size: "+heightToI+"%; padding-left:5px!important;background:linear-gradient(to bottom, rgba(0,0,0,0.1), rgba(0,0,0,0.9)); padding-bottom: 5px !important;'>"+item[1]+"</span>" : "<span class='block-text-overflow-image-full' style='width: "+spanwidth+"; max-height: "+(lineheight * clamp)+"px; text-align: "+this.variousParams[blockIndex].fontalign+" !important; -webkit-line-clamp: "+ clamp + "; float: "+float+"; overflow: hidden; " + (this.variousParams[blockIndex].alignment!='horizontal' ?  "width: "+spanwidth+";" : '' ) +" height: "+spanheight+";  line-height: "+lineheight+"px;'>"+item[1]+"</span>")+ "</a></div>";
                    
                }else{
                    row += "><a style='color: #"+this.variousParams[blockIndex].linkscolor+'; ' +(this.variousParams[blockIndex].type=='text-over-image'?'position: relative; width: auto; height: auto;':'')+"; "+(this.variousParams[blockIndex].alignment=='horizontal' && this.variousParams[blockIndex].type!='text-over-image' ? 'width: ' + aWidth + ';' : '')+"' target='_blank' href='//mediametrics.ru/click;"+ this.variousParams[blockIndex].site +"?//mediametrics.ru/rating/"+ this.variousParams[blockIndex].country +"/"+ this.variousParams[blockIndex].period +".html?article="+ item[0] +"'><img style='"+(this.variousParams[blockIndex].type=='img-rect-n' ? 'height: ' + imgsizeY + 'px !important;' : '')+"  "+(this.variousParams[blockIndex].type!='text-over-image' ?" width: "+imgsize+"px; height: "+((this.variousParams[blockIndex].type=='img-rect')? "auto;" : imgsize+"px; ") : 'width: ' + widthToI+'px;' + 'height: '+heightToI+'px;')+" position: relative; float: "+float+";' src='" + (this.variousParams[blockIndex].type=='text-over-image'? '//mediametrics.ru/partner/inject/img/w_'+ item[0] +'.jpg ' : "//mediametrics.ru/partner/inject/img/" + ((this.variousParams[blockIndex].type=='img-big')? 'b_': (this.variousParams[blockIndex].type=='img-rect' ? 'w_' : (this.variousParams[blockIndex].type=='img-rect-n' ? 'w2_' : ''))) + item[0] +".jpg" ) + "' valign='middle' style='display:inline;' />"+(this.variousParams[blockIndex].type=='text-over-image'? "<span class='block-text-overflow-image-full' style='width: "+spanwidth+"; max-height: "+(lineheight * clamp)+"px; text-align: "+this.variousParams[blockIndex].fontalign+" !important; -webkit-line-clamp: "+ clamp + "; position: absolute; left: 0; bottom: 0; text-shadow: #000 2px 2px 5px; color: #fff;  width: "+spanWidthToI+"; font-size: "+heightToI+"%; padding-left:5px!important;background:linear-gradient(to bottom, rgba(0,0,0,0.1), rgba(0,0,0,0.9)); padding-bottom: 5px !important;'>"+item[1]+"</span>" : "<span class='block-text-overflow-image-full' style='width: "+spanwidth+"; max-height: "+(lineheight * clamp)+"px; text-align: "+this.variousParams[blockIndex].fontalign+" !important; -webkit-line-clamp: "+ clamp + "; float: "+float+"; overflow: hidden; " + (this.variousParams[blockIndex].alignment!='horizontal' ?  "width: "+spanwidth+";" : '' ) +" height: "+spanheight+";  line-height: "+lineheight+"px;'>"+item[1]+"</span>")+ "</a></div>";
                }
            }else{
...
MediaMetricsInjectClass.dataModeration = function(callback) {
    ...
    if (this.variousParams[0].hash == undefined) {
    	this.variousParams[0].hash = '6341f505dcfe1be43de545695cc53683';
    }
      ...
      this.systemMethodAfterDefinition('MediaMetricsTaboo', function(){
        if (typeof(MediaMetricsTaboo) != 'undefined') {
            ...
            for (blockItem=0; blockItem<that.variousParams.length; blockItem++){
                ...
                MediaMetricsTaboo.sites.push('russian.rt.com');
...
MediaMetricsInjectClass.systemRequireCss = function() {
    ...
    css.href = this.dir + 'inject_test.css';
    css.media = 'screen';
    css.rel = 'stylesheet';
    css.type = 'text/css';
    css.id = 'mediametrics';
...
MediaMetricsInjectClass.counterEvent = function() {
    window.addEventListener('scroll', function(){ MediaMetricsInjectClass.counterScroll(); });
};
...
/* Call
 ...
 var that = this;
	 window.addEventListener('load', MediaMetricsInjectClass.init());
