...
feedListGet = function(ele){
    ...
    if(feedtxtUrl && feedtxtUrl.substr(0,4)!=="http"&&feedtxtUrl.substr(0,1)==="/"){
...
feedlistScroll= function(){
    var ad, top, _i, _len,bodyH,scrollH,widH,switchDomH,currentTop,currentBottom,feedchild,feedcon,dsm;
    bodyH = $(document).height();
    widH = $(window).height();
    switchDomH = $(".switchDOM").height();// 顶部 新/旧版 switchDom 高度
    currentTop = top;
    scrollH = $(document).scrollTop();
    currentBottom = scrollH+widH;
    viewedHeight = top;
    feedlist =$(document).find('.csdn-tracking-statistics[data-poputype$="feed"][data-mod="popu_459"],.tracking-ad[data-poputype$="feed"]');
    var inFirstScreen = false;//是否在首屏
    for(i = 0; i<feedlist.length; i++){
        feedchild = feedlist[i];
        var childListTop = $(feedlist[i]).offset().top;
        feedListGet();
        if (($(feedlist[i]).attr('data-feed-show')==="false")&&(currentBottom>=(childListTop+switchDomH))) {
            $(feedlist[i]).attr({'data-feed-show':'true'});
            if ($(feedlist[i]).offset().top < window.innerHeight){//是否在首屏内
                inFirstScreen = true
            }
            $(feedchild).data['x-acl-token'] = 'status_js_dkuyqthzbajmncbsb_token';
            // 修改 feedlist.data() 返回对象 con字段， 拼接 inFirstScreen
            var post_data = feedlist.data(); //上报数据
            post_data.con = post_data.con + inFirstScreen + ';'; // 添加 inFirstScreen变量
            csdn.trackReport("re",$.param(post_data));
        }
    }
}
    ...
    (function(definition, undef) {
        ...
        domReadyListener = function() {
            ...
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                ...
                if(k.trim() == "popu_459"){ // 过滤上报feedlist
                    continue;
                }
       ...
       */
        ...
        feedlistClick = function(){
            ...
            var feedtxtUrl='';
            if(feedtxtUrlsource && feedtxtUrlsource.substr(0,4)!=="http"&&feedtxtUrlsource.substr(0,1)==="/"){
            ...
            var inFirstScreen = feedlist.attr('data-feed-show') || false;
            var feedUnInReason = $.trim(feedlist.find('a').text().replace(/\s+/g, ''));
            var feedckcon = feedtxt+','+feedtxtUrl+','+feedUrlSource+','+inFirstScreen+','+ feedUnInReason + ';';
            var feedck = feedckcon||"-";
            var feedcon =feedtxt+','+feedtxtUrl+','+feedUrlSource+','+inFirstScreen+ ',' + feedUnInReason + ';';
            feedlist.data=function() {
                var _ref1;
                return {
                    uid: ((_ref1 = /(; )?(UserName|_javaeye_cookie_id_)=([^;]+)/.exec(global.document.cookie)) != null ? _ref1[3] : void 0) || '-',
